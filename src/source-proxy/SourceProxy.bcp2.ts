...
function ImmutableProxy<T extends object>(source: T, handler: ImmutableProxyHandler<T> = {}): T {
  ...
  return Object.assign(
    new Proxy(source as any, _handler),
...
export function ConstructSourceProxy(proxy: ISourceProxy, everyKeyAsSource: boolean = true): void {
  ...
  (proxy as ISourceProxyInternal)[SOURCE_PROXY_PRIVATE].templateProxy = CreateSourceProxyTemplateProxy(proxy);
  (proxy as ISourceProxyInternal)[SOURCE_PROXY_PRIVATE].dataProxy = CreateSourceProxyDataProxy(proxy);
  (proxy as ISourceProxyInternal)[SOURCE_PROXY_PRIVATE].everyKeyAsSource = Boolean(everyKeyAsSource);
}

export function CreateSourceProxyTemplateProxy(proxy: ISourceProxy, path: PropertyKey[] = []): any {
  return new Proxy(Object.create(null), {
...
export function CreateSourceProxyDataProxy(proxy: ISourceProxy, path: PropertyKey[] = [], data: object = Object.create(null)): any {
  return Object.assign(new Proxy(data, {
...
function SourceProxyEmitValue(proxy: ISourceProxy, path: PropertyKey[], value: any): void {
...
function SourceProxyIs$$(propertyName: PropertyKey): boolean {
  return (typeof propertyName === 'string') && propertyName.startsWith('$$');
}

function SourceProxyIsSourceKey(proxy: ISourceProxy, propertyName: PropertyKey): boolean {
...
export function SourceProxyGetTemplateValue(proxy: ISourceProxy, path: PropertyKey[], propertyName: PropertyKey): any {
  ...
  if (is$$ || isSourceKey) {
    ...
    return CreateSourceProxyTemplateProxy(proxy, path.concat(propertyName));
  }
}
...
export function SourceProxySetDataValue(proxy: ISourceProxy, path: PropertyKey[], target: any, propertyName: PropertyKey, value: any, receiver: any): boolean {
  if (SourceProxyIs$$(propertyName)) {
    ...
    if ((typeof value === 'object') && (value !== null)) { // 'value' is an object
      ...
      if (source === void 0) {
        value = CreateSourceProxyDataProxy(proxy, _path, value); // cast 'value' to proxy
      } else {
...
class SourceProxy implements ISourceProxy {
  ...
  set data(value: any) {
    ReflectObjectProperties(((this as unknown) as ISourceProxyInternal)[SOURCE_PROXY_PRIVATE].dataProxy, value);
  }
...
function testSourceProxy2() {
  ...
  const proxy = new SourceProxy();
  // proxy.template.a.$$.pipeTo((value: any) => {
  //   console.log('a emit', value);
  // }).activate();

  // proxy.template.b.pipeTo((value: any) => {
  //   console.log('b emit', value);
  // }).activate();
  // proxy.getTemplateSource(['b', 2]).pipeTo((value: any) => {
  //   console.log('b emit', value);
  // }).activate();
  ...
  // }).activate();
  //
  proxy.template.b[0].pipeTo((value: any) => {
